...
- **Decision:** Run the preview-only queue hourly at minute 30, one new candidate maximum per run. Pending candidates do not block others. Approval prompts enumerate `Install A`, `Install B`, and so on; bare `install` never authorizes a background write.
...
- **Decision:** Resolve only explicit label-bearing approvals such as `Install A` or `Install B` to an exact displayed, queue-bound file and hash in the same task. Never infer approval from bare `install`. After current backup verification, prefer `Complete-ApprovedBackgroundInstall.ps1` so one mutex-held workflow consumes queue hashes, invokes the guarded installer, records proof without shell-quoting hazards, refreshes the installed-content fingerprint, and validates terminal state.
