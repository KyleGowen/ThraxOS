...
## 2026-08-06 three selected individual-song installations

- **Owner-confirmed:** Install recommendation items 1, 2, and 4: The Midnight's `SUNSET (Michael Cassette Remix)`, Elton John & Dua Lipa's `Cold Heart (PNAU Remix)`, and Dua Lipa's `Physical` in `Misc. Collected`.
- **Observed:** The three Zenius-I-vanisher songs are installed in matching folders under `C:\Games\ITGmania\Songs\Misc. Collected`. `SUNSET` contains two simfiles and one audio file; the other two each contain one simfile and one audio file.
- **Observed:** ZIP size/SHA-256 values were 2,703,822 bytes / `B9A36C1BBB6B457AFE2686F68FCA6A41D51EE35C8E963D9B1336A45E6B324F3D`, 6,168,184 / `A5ED1D84D7AC90314E3B30F243E84A3B29E7256A7C463C3390C8FD64D79CDE3E`, and 5,267,433 / `2A2294F30511246E092900BF20556562EF333F1C0315B13B05BA2D3DDC3F45EF` respectively. Defender found no threats in any archive or extracted tree; no unsafe path, executable payload, destination collision, or verified metadata mismatch was accepted. All task staging was removed and ITGMania remained closed.

## 2026-08-14 StepManiaX and menu input repair

- **Observed:** The NES controller's earlier Player 1 mapping had replaced the StepManiaX directions. The active ITGMania session enumerated `StepManiaX` as the only joystick (`Joy1`), whose verified directional buttons are 2/4/6/8; the stale NES map overlapped only on button 6, explaining the owner report that only Right worked.
- **Observed:** With ITGMania closed and a current successful backup, the active roaming `Keymaps.ini` was changed only to restore StepManiaX as `Joy1_B2/B4/B6/B8` and pair those directions with keyboard W/A/S/D. Keyboard Enter is Start/confirm and Q is Back. Dedicated menu and Select entries are blank. Hash-verified sibling rollback copies of both changed files were created.
- **Decision applied:** `AutoMapOnJoyChange=0` in active `Preferences.ini` prevents a future controller arrival from automatically overwriting the verified pad mapping. The paired NES controller remains unavailable as an ITGMania DirectInput device and is not bound until Windows exposes it before game launch.

- **Observed:** Once Windows exposed the NES controller and ITGMania enumerated it as `Joy2`, its physical Select (`Joy2_B9`) was added as the second binding for both Player 1 Left and Right. StepManiaX remains the first binding (`Joy1_B4`/`Joy1_B6`), so one NES Select press invokes the Simply Love Left+Right submenu chord without changing pad gameplay. Keyboard A/D moved to dedicated menu-left/menu-right alongside the NES D-pad; W/S, Enter, and Escape remain keyboard navigation/confirm/back fallbacks.
