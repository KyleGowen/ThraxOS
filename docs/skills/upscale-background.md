...
Installation prompts enumerate label-bearing commands such as `Install A` or `Install B`; a single-candidate prompt asks for `Install A`. Bare `install` is intentionally rejected because it does not identify an exact displayed file and hash. A later explicit label may supersede a prior rejection only after that exact staged candidate is rebound and all source, simfile, preview, backup, and closed-game checks pass again.
...
4. Test installation only on a disposable fixture. Confirm label-explicit approval (`Install A`, `Install B`, and so on), task-local queue binding, `-BackupVerified`, expected hashes, closed-game checks, 1920 x 1080 at `16:9`, one frame, encoding matching the referenced extension, unchanged simfiles/reference, verified rollback, refreshed terminal fingerprint, and exact live/proof hash agreement. Confirm bare `install` is not treated as approval.
