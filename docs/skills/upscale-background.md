...
Approval and owner-decision messages lead with canonical simfile `ARTIST — TITLE` and append a nonblank subtitle. They also identify the pack and song folder, source quality tier and dimensions, candidate label and output dimensions, and any useful nonblank transliteration, genre, or credit metadata. Missing metadata is not inferred from filenames or artwork. Artist and title also appear in the inbox item so queued work is recognizable without opening the task.
...
The queue and learning helpers read and write `background-upscale-queue.json` explicitly as UTF-8 without a BOM. This avoids Windows PowerShell's legacy implicit decoding, which can repeatedly corrupt non-ASCII history and inflate the ledger on each rewrite.

## Reproduce and verify

1. Copy the complete skill and adapt approved paths/display convention.
2. Run `Test-BackgroundQueue.ps1`, then validate the skill. Confirm the suite accepts empty `BGCHANGES`, rejects active changes, orders quality tiers, records review-only sources, preserves installed-preview history, and round-trips non-ASCII history exactly.
