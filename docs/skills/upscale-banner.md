...
- `scripts/Install-ApprovedBanner.ps1`: hash-binds, backs up, and installs the approved preview; it refuses an open game or changed preview/source/simfile, reports full hash evidence, and requires an explicit contained fallback original when the referenced target is missing.
...
- `scripts/Test-BannerInstallWorkflow.ps1`: disposable fixture test for expected-hash refusal, rollback/hash evidence, installed dimensions, unchanged simfile, and cross-process queue-lock waiting.
