...
2. Run `Test-BackgroundQueue.ps1`, validate the skill, then initialize the queue with `Update-BackgroundQueue.ps1 -Refresh`. Confirm the disposable Unicode history fixture survives repeated helper rewrites exactly.
...
4. Test one preview-only run. Confirm empty-tag acceptance, active-change exclusion, review-only nonselection, severity-first ordering, one selected fingerprint, labeled source/result rendering, canonical artist/title plus useful identification metadata in the approval message and inbox item, recorded preview hash, no live write, an exact install question, and that skipped or attempt-exhausted fingerprints do not re-enter selection unless their content or owner decision changes.
