...
2. Run `Test-BackgroundQueue.ps1` and `Test-BackgroundInstallWorkflow.ps1`, validate the skill, then initialize the queue with `Update-BackgroundQueue.ps1 -Refresh`. Confirm Unicode history survives repeated rewrites, mutex serialization works, stale hashes fail closed, and exact/normalized installs preserve terminal proof.
