...
Interactive installation follow-ups must record the pending fingerprint as installed only after the helper proves the live banner equals the installer's deterministic rendering of that exact preview. If installation created a missing referenced target from a contained fallback, the helper permits only that expected source-path transition and proves the new target rather than the unchanged fallback. A subsequent refresh migrates the terminal decision, history, and installed-source hash to the installed-content fingerprint. `-RecordInstalledContent` recovers an interrupted/legacy refresh or idempotently repairs a missing installed-source hash; both paths require the same proof and installed re-proof must not duplicate a decision.
...
5. Test one preview-only run. Confirm one fingerprint, independently decoded and normalized output with intended opacity, visible `Before`, every viable labeled After option, pending fingerprint/hash state, no live write or restart, a clear exact-option approval request, fingerprint-scoped good-as-is skipping, and a documented retrospective that does not invent owner feedback.
6. In a disposable song/queue fixture, test approved installation recording and refresh preservation. Confirm exact installed-render proof succeeds, a mismatched live source is rejected, and terminal preview/decision history survives the fingerprint change.
