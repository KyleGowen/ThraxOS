...
if(-not $target.StartsWith($root,[StringComparison]::OrdinalIgnoreCase)){throw '#BANNER resolves outside the song directory.'}; if(-not(Test-Path -LiteralPath $target -PathType Leaf)){throw 'Referenced banner does not exist.'}
