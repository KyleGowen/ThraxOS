...
4. Always render `Before`, including failures, and every viable `After` (`After A`, `After B`, etc.). Run `scripts/Get-ImagePresentation.ps1 -Path <image>` and list each image's decoded dimensions and aspect ratio as both reduced `W:H` and decimal `W/H:1` (for example, `640 x 480 — 4:3 (1.3333:1)`). If an inline display copy is needed, label its presentation separately from the exact install candidate. Bind each installable candidate to its exact file and SHA-256. End the approval request with explicit label-bearing choices such as `Install A` or `Install B`; when only one candidate exists, ask for `Install A`. Never ask for or accept bare `install` because it does not identify an exact candidate.
5. Make every approval or owner-decision message immediately identifiable. Read canonical metadata from the exact simfile and lead with the full nonblank `#ARTIST — #TITLE`, appending `#SUBTITLE` when present. Include the pack, song folder, quality tier, Before dimensions/aspect ratio, each candidate label with output dimensions/aspect ratio, and other useful nonblank simfile fields such as transliterations, genre, and credit. Never infer missing metadata from artwork or folder names. Carry artist and title into the inbox item title or summary.
...
1. Install only after the owner explicitly names a displayed candidate with `Install A`, `Install B`, and so on. The label must resolve to one exact staged file and SHA-256 in the same task; never infer approval from bare `install`, global pending order, or an unlabeled artifact. A later explicit labeled choice may override the latest candidate or a prior rejection only after rebinding and rechecking that exact file. Recheck current backup evidence before any live write.
...
Run `scripts/Test-BackgroundQueue.ps1` after changing discovery, eligibility, state migration, or selection behavior. Run `scripts/Test-BackgroundInstallWorkflow.ps1` after changing presentation reporting, locking, approval resolution, installation, rollback, hash proof, or installed-state migration.
