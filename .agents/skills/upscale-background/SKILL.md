...
1. Resolve bare `install` only to the latest explicitly labeled, queue-bound After in the same task. Recheck current backup evidence; do not infer approval from global pending order.
