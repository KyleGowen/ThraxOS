...
- Build every local inline image block with `Format-BackgroundPreviewMarkdown.ps1`: use its forward-slash, percent-encoded destination verbatim and keep its native full-resolution link. Never put an angle-wrapped Windows backslash path inside `![...]()`. If the client still shows a placeholder, verify the file/hash and retry from a short-path byte-identical display copy without regenerating or changing the queue binding.
